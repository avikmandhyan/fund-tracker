...
def upload_bhav():
    """Upload a bhavcopy CSV directly — used when NSE blocks server-side downloads."""
    ...
    if not rows:
        return jsonify({"error": "no rows parsed"}), 400
    # Detect date from data
    date_str = (rows[0].get("TradDt") or rows[0].get("BizDt") or "").strip()[:10]
    try:
        d = datetime.date.fromisoformat(date_str)
    except:
        d = trading_day(0 if label=="today" else 5 if label=="week" else 22)
    # Save to cache
    cp = cache_path(d)
    cp.write_text(text, "utf-8")
    _load_csv(text, label, d)
    with db_lock:
        count = len([k for k in (price_db_today if label=="today" else price_db_week if label=="week" else price_db_month) if not k.startswith("INE")])
    return jsonify({"status": "ok", "label": label, "date": d.isoformat(), "stocks": count})
